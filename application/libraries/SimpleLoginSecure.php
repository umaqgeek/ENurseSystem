...
class SimpleLoginSecure
{
	...
	function login($u_username = '', $u_password = '') 
	{
		...
		if($this->CI->session->userdata('ns_username') == $u_username)
			return true;
		
		
		//Check against user table
		$this->CI->db->where('ns_username', $u_username); 
		$this->CI->db->where('ns_password', $u_password); 
//		$this->CI->db->where('me_activation_status', 2); //make sure the email is activated
		$query = $this->CI->db->get_where($this->user_table);
